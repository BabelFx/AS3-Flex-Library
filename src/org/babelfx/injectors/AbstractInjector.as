...
package org.babelfx.injectors
{
	...
	public class AbstractInjector extends EventDispatcher implements IMXMLObject {
			...
			if (target != null) {
				if ((target is String) && _registry[target])  {
					...
					for (var key:String in _registry) {
						var it : org.babelfx.proxys.ResourceSetter = _registry[key] as org.babelfx.proxys.ResourceSetter;
				...
				} else if (target is org.babelfx.proxys.ResourceSetter) {
					self::removeItem(target as org.babelfx.proxys.ResourceSetter);
				}
			}
			else if (target == null) {
		 ...
		 * 
		 */
		private function addItem(src:Object):org.babelfx.proxys.ResourceSetter {
			if (src == null) return null;
			
			var results     :org.babelfx.proxys.ResourceSetter= null;
			...
			if ((target != null) && (property != "") && (key != "")) {
				...
				if (_registry[key] == undefined) {
					...
					results = new org.babelfx.proxys.ResourceSetter(target,key,property,state,type,parameters,bundle);
		 ...
		 * 
		 */
		self function addItem(map:org.babelfx.proxys.ResourceSetter):org.babelfx.proxys.ResourceSetter {
			if (map != null) {
		...
		self function removeItem(map:org.babelfx.proxys.ResourceSetter):void {
			if (map != null ) {
		...
		protected function buildRegistry(target:Object):void {
			...
			for (var j:int=0; j<items.length; j++) {
				...
				if (it is org.babelfx.proxys.ResourceSetter) 	 self::addItem(it as org.babelfx.proxys.ResourceSetter);		// Add typed item
		...
		protected function assignResourceValues(viewState:String=""):Boolean {
			...
			for (var key:Object in _registry) {
				var map : org.babelfx.proxys.ResourceSetter = (_registry[key] as org.babelfx.proxys.ResourceSetter);
		 ...
		 * 
		 */
		protected function assignResourceValuesTo(map:org.babelfx.proxys.ResourceSetter):void {
		 ...
		 * 
		 */
		private function findItemByMap(map:org.babelfx.proxys.ResourceSetter):Object {
		...
		private function isValidTargetState(map:org.babelfx.proxys.ResourceSetter):Boolean {
			var results 	 : Boolean = true;
			var desiredState: String  = map.state;
			
			function listenStateChanges(map:org.babelfx.proxys.ResourceSetter):UIComponent {
		 ...
		 * 
		 */
		private function resolveEndPoint(map:org.babelfx.proxys.ResourceSetter):Object {	   	 
			var results : Object = null;
		 ...
		 * 
		 */
		private function resolveProperty(map:org.babelfx.proxys.ResourceSetter):String {
			return InjectorUtils.resolveProperty(map.property);
		}
		
		private function logError(map:org.babelfx.proxys.ResourceSetter,errorType:String,node:String=null ):void {
